  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full h-full bg-gradient-to-r from-fuchsia-500 to-cyan-500 p-[2px] rounded-2xl shadow-card"
      >
        <div className="max-w-sm h-full flex flex-col items-center rounded-2xl shadow-lg bg-gradient-to-t from-violet-950 to-black translate-z-60">
          <img
            className="w-[95%] h-[200px] rounded-lg mt-2"
            src={project.image}
            alt={t(`projects.${project.name}.title`)} // Translate alt text
          />
          <div className="px-4 pt-3">
            <div className="font-bold text-xl mb-2">
              {t(`projects.${project.name}.title`)} {/* Translate project name/title */}
            </div>
            <p className="text-secondary text-sm">
              {t(`projects.${project.name}.description`)} {/* Translate description */}
            </p>
          </div>
          <div className="w-full flex justify-start gap-2 mt-2 px-4">
            ...
            <a
              ...
              className={project.hasDemo ? "w-1/2" : "w-full"}
            >
              <button
                type="button"
            ...
            {project.hasDemo && (
              ...
              >
                ...
                >
                  ...
                  {t('projects.demoButton')} {/* Translate "Demo" */}
                </button>
              </a>
            )}
          </div>
          <div className="px-4 py-4 flex flex-wrap justify-center gap-1">
            {project.tags.map((tag, key) => (
              <span
                key={key}
                className={`bg-gray-200 rounded-full text-xs px-3 py-1 font-semibold ${tag.color}`}
              >
                #{t(`projects.tags.${tag.name}`)} {/* Translate tags */}
              </span>
            ))}
          </div>
        </div>
